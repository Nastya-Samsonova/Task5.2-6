package ru.vsu.cs.samsonova_a_g;
...
public class SimpleBinaryTree<T> implements BinaryTree<T>
{
    ...
    public ArrayList<Integer> findMaxNodesLevels()
    {

        ArrayList<Integer> levelsList = new ArrayList<>();

        BinaryTreeAlgorithms.Visitor<T> visitor = (value, level) ->
        {
            try
            {
                levelsList.set(level, levelsList.get(level) + 1);
        ...
        BinaryTreeAlgorithms.preOrderVisit(this.root, visitor);
        return findMaxValueIndexes(levelsList);
    }

    private ArrayList<Integer> findMaxValueIndexes(ArrayList<Integer> levelsList)
    {
        ArrayList<Integer> maxIndexes = new ArrayList<>();

        int max = levelsList.get(0);
        for (int value : levelsList)
        {
            if (value > max)
            {
                max = value;
        ...
        for (int n = 0; n < levelsList.size(); n++)
        {
            if (levelsList.get(n) == max)
            {
                maxIndexes.add(n);
            }
        }
